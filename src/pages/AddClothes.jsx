...
import { Camera, Type, Check, Loader2, X } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen px-6 pb-28 pt-8">
      ...
      <div className="mb-5 flex rounded-2xl p-1" style={{ background: 'var(--color-mist)' }}>
        {[{ key: 'photo', label: 'Upload photo', icon: Camera }, { key: 'text', label: 'Text only', icon: Type }].map(({ key, label, icon: Icon }) => (
          <button
            key={key}
            onClick={() => setMode(key)}
        ...
        <div className="mb-6">
          ...
          {!image ? (
            <button
              ...
              className="flex h-56 w-full flex-col items-center justify-center gap-2 rounded-3xl border-2 border-dashed"
              style={{ borderColor: 'var(--color-mist-2)', color: 'var(--color-ink-soft)' }}
            >
              <Camera size={28} />
            ...
            <div className="relative">
              <img src={image} alt="Uploaded outfit" className="h-72 w-full rounded-3xl object-cover shadow-lg" />
              <button
                onClick={() => { setImage(null); setColorTouched(false); setColor({ rgb: [111, 183, 232], hex: '#6FB7E8' }) }}
                className="absolute right-3 top-3 rounded-full p-1.5 backdrop-blur-md"
              ...
              {extracting && (
                <div className="absolute inset-0 flex items-center justify-center rounded-3xl backdrop-blur-sm" style={{ background: 'rgba(11,37,69,0.25)' }}>
      ...
      {/* Color — auto-extracted from photo (editable), or fully manual for text-only entries */}
      <div className="mb-6">
